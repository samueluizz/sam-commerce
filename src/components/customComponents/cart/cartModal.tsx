...
export function CartModal({ dropdown }: { dropdown?: boolean }) {
  ...
  // Simulação de subtotal e entrega para exemplo
  ...
  return (
    <div className='fixed inset-0 bg-black/70 z-50 flex justify-center items-center'>
      <div className='w-full max-w-sm bg-neutral-900 text-white rounded-2xl shadow-2xl p-4 flex flex-col gap-4'>
        {/* Cabeçalho */}
        <div className='flex items-center justify-between mb-1'>
          <h2 className='text-xl font-bold'>Your Cart</h2>
          <span className='bg-neutral-800 text-xs px-3 py-1 rounded-full'>
            {totalItems} items
          </span>
        </div>
        ...
        <button
          onClick={toggleCart}
          className='absolute top-4 right-6 text-neutral-400 hover:text-white text-lg'
        >
          ×
        </button>
        ...
        <div className='flex flex-col gap-4 max-h-72 overflow-y-auto'>
          {cart.length === 0 ? (
            ...
            cart.map((item, idx) => (
              <div
                key={item.id || idx}
                className='flex gap-3 items-center bg-neutral-800 rounded-lg p-2'
              >
                ...
                <div className='flex-1 flex flex-col gap-1'>
                  ...
                  <div className='flex items-center gap-2 mt-1'>
                    <Button
                      size='sm'
                      variant='outline'
                      className='px-2 py-0.5 h-7 text-white border-neutral-600 bg-neutral-700 hover:bg-neutral-600'
                      onClick={() => updateQuantity(item.id, item.quantity - 1)}
                    >
                      –
                    </Button>
                ...
                <div className='flex flex-col items-end gap-1'>
                  ...
                  <button
                    className='text-neutral-400 hover:text-red-500 text-sm mt-1'
                    onClick={() => removeFromCart(item.id)}
                    title='Remove'
                  >
                    <FaTrash />
        ...
        {cart.length > 0 && (
          <div className='flex flex-col gap-2 border-t border-neutral-700 pt-4'>
            ...
            <div className='text-xs text-neutral-400 flex items-center gap-1 mt-2'>
              <span>🚚</span> Free shipping on orders over $200
            </div>
            <Button
              size='lg'
              className='w-full mt-2 bg-white text-neutral-900 font-bold hover:bg-neutral-200'
            >
              Checkout
            </Button>
            <Button
